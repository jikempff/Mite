...
### Kinetics
- **Net Kinetics** — moves a scissor-jointed asymptotic net as a mechanism (Schikore, Schling, Oberbichler & Bauer 2020; Wan, Crolla & Schling 2025): constant joint spacing along the laths, laths perpendicular to the moving surface normal, rest bend kept with a chosen stiffness; driven by fixed / sliding supports, moved points or actuator cables, with a `Fold` slider and per-state drift / deviation / miss diagnostics. Every state is read elastically: bending and twist utilization of the lath section, strain energy Π = ½∫(GJτ² + EIκ²) ds and the natural (least-energy) state along the motion. Exact on the doubly ruled hyperboloid grid (1e-12), settles traced nets in one step
...
- Elastic reading of every kinetic state (`Kinetics/KineticStrain`, appended inputs Upright / Width / Thickness / MaxStrain / Modulus / Density / Shape / Section and outputs Utilization, LathUtil, Energy, Natural, Buildable on Net Kinetics): curvatures in the Darboux frame of the moving net (kn, kg from the lath's turning at each joint; τg from the normal's rotation between joints, exactly ∫τg ds on an asymptotic lath), strains with the section's fibre distances and Saint-Venant twist length as in Lath Analysis, Schikore et al.'s strain energy Π = ½∫(GJτ² + EIκ²) ds plus a self-weight potential, and the least-energy state along the motion as the natural configuration. Free laths twist uniformly between hinges and carry no torque beyond the last one (twist residuals on subdivision nodes and tails). Exact checks: hyperboloid rods turn their normal by atan(s/(ρ sin β)), so the joint-to-joint twist is exact and the energy is the Jensen lower bound of ½GJ∫(√−K)² ds (within 1.2 %); a circular lath splits into kn or kg exactly with energy ½EIκ²L; the planar lazy tongs store nothing. Catenoid net: utilization 1.37 at rest (Beltrami–Enneper twist √−K·t/√3 at the throat) → 1.18 as the ring closes, natural state at the closed end.
