...
"Principal Curvature": dict(demo="torusK", demoMode="k1", expect=[
    ...
    checks=[("Sphere K1, K2 within 2% of 1", "[Math.abs(N.sphere.k1-1)<0.02 && Math.abs(N.sphere.k2-1)<0.02, 'k1 = '+N.sphere.k1+', k2 = '+N.sphere.k2+' on a 24-division unit sphere']"),
            ...
            ("Cylinder (R = 1): k1 = 1, k2 = 0 — exact inside, the jet-fitted border rows within 1 % / 1e-4", "[N.cylinder.k1Err<0.01&&N.cylinder.k2Max<1e-4,'max |k1 − 1| = '+N.cylinder.k1Err+', max |k2| = '+N.cylinder.k2Max.toExponential(1)+' over all vertices of a 64 × 32 cylinder (border rows included)']")]),
...
"Asymptotic Net": dict(demo="asym", shapes=[("asym", "saddle"), ("asymHyp", "hyperboloid"), ("asymCat_catenoid", "catenoid"), ("asymCat_enneper2", "Enneper 2"), ("asymCat_enneper3", "Enneper 3"), ("asymCat_bilinear", "ruled"), ("asymCat_schwarzD", "Schwarz D"), ("asymCat_catenoidWeb", "catenoid · web")], expect=[
    ...
    "Layout (appended input): 0 fills the region with evenly spaced curves (Jobard–Lefer: new curves inserted where the spacing opens, stopped on a neighbour where it closes — T-junctions); 1 is a web from the border, 2 a web from the seed cross: seeds exactly Spacing apart (measured across the curves) along the border or the crossing curve through the seed, every curve traced border to border, none ever stopped on a neighbour. Asymptotic curves are never equidistant except on special surfaces — on the catenoid neighbours separate as cosh z — so the web shows the true spacing and the fill trades continuity for evenness. Built asymptotic gridshells are webs (Schling, Hitrec & Barthel 2017; Schling 2018 §4.3).",
    "Gap filling (webs): wherever the region is still farther than 0.75 Spacing from every curve — behind a K = 0 line the border cannot see across, a band a single seed cross does not reach, an island, a surface without border — a further seed cross is laid exactly one Spacing from the nearest curve, perpendicular to it, so the ladder continues at the right spacing and the leftover ends where the gap closes. A web curve ends early only where it would touch a curve of its own family (closer than 0.15 Spacing, within 30°): near a flat point the curves on either side of a separatrix leave together (monkey saddle: 5 of 150 ends).",
    "Border accuracy: the curvature tensor at the three border rows is no longer the one-sided normal-difference estimate (8° / 4° / 1° direction error on a catenoid rim) but an osculating-jet fit of the two inner rings (Cazals & Pouget 2005) with the border row extrapolated from them: ≤ 0.7° / 0.02° / 0.02°. A traced curve now stays within 0.03 edge of the closed-form asymptotic line instead of 0.35 edge, because the whole error used to be made in the last rows and then carried across the surface.",
    "Curves meet the border along their own direction: the last segment carries no hook and the trace never crawls along the edge, also on staircase borders of trimmed quad meshes (Weaverbird-style); fairing keeps the end segments fixed.",
    ...
    checks=[("No family mixing", "[N.asym.wrongFamily===0,N.asym.wrongFamily+' of '+N.asym.samples+' samples follow the other family']"),
            ...
            ("Web coverage (family A): with gap filling both webs reach as much of the usable region as the evenly spaced fill (what stays uncovered is too small to hold a curve two spacings long)", "(()=>{const r=N.webCoverage;const ok=r.every(x=>x.border.uncovered<=x.fill.uncovered+1.5&&x.cross.uncovered<=x.fill.uncovered+1.5);return [ok,r.map(x=>x.name+': fill '+x.fill.curves+' curves '+x.fill.uncovered+' % uncovered '+x.fill.tEnds+' T · border web '+x.border.curves+' / '+x.border.uncovered+' % / '+x.border.tEnds+' T · cross web '+x.cross.curves+' / '+x.cross.uncovered+' % / '+x.cross.tEnds+' T').join('; ')];})()"),
            ("Web layout (Layout = 1) on the catenoid: every curve rim to rim, no T-junctions, within 0.05 edge of the closed form u ∓ v = const; direction field within 1° of 45° to the meridians", "[N.asymWeb.floatingEnds===0&&N.asymWeb.tEnds===0&&N.asymWeb.boundaryEnds===N.asymWeb.ends&&N.asymWeb.worstOffset<0.05*N.asymWeb.edge&&N.asymWeb.worstDirDeg<1.0,N.asymWeb.a+'+'+N.asymWeb.b+' curves, '+N.asymWeb.boundaryEnds+'/'+N.asymWeb.ends+' ends on the rims, worst offset from the exact curve '+N.asymWeb.worstOffset+' ('+(N.asymWeb.worstOffset/N.asymWeb.edge).toFixed(2)+' edges), direction error mean '+N.asymWeb.meanDirDeg+'° worst '+N.asymWeb.worstDirDeg+'° (was 0.4° / 8° before the border repair), '+N.asymWeb.ms+' ms']"),
